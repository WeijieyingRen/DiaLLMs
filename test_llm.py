...
def test_instance(tokenizer, model, input_tensor, attention_mask, ref_answer, gen_kwargs, score_model):
    # only takes one input instance
    model.eval()
    with torch.no_grad():
        response_tensors = model.generate(input_ids = input_tensor.unsqueeze(0).to('cuda'), attention_mask = attention_mask.unsqueeze(0).to('cuda'), **gen_kwargs,
                )
        response_tensors = response_tensors[:, attention_mask.sum()-1:]
...
for sample in tokenized_datasets:
    ...
    similarity_all = test_instance(tokenizer, model, sample['input_ids'], sample['attention_mask'], sample['Ans'], generation_config, score_model=score_model )#sample['category']
